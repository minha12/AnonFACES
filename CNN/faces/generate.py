...
def generate_object(parameters, output_dir, batch_size=32,
        extension='jpg', model_path=False, yaml=True):
    """
    ...
    if not yaml:
        ...
        params['em'] = parameters[2]
        model = parameters[3]
        num_ids = parameters[4]
        dataset = parameters[5]
    else:
        parser = GenParser(parameters)
        print("Loading model...")
        time1 = time.time()
        ...
        print('Loading model time: ', time2)
        try:
            params = parser.parse_params()
        except RuntimeError as e:
            print("Error: Unable to parse '{}'. Encountered exception:".format(parameters))
            print(e)
            return
    params['dataset'] = dataset
    params['num_ids'] = num_ids
    #print('---** params for GNN: ', params)
    inputs = parser.gen_inputs(params)
    #print('---** inputs for GNN: ', inputs)
